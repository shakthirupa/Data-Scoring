...
import { ShieldCheck, Settings, Eye, EyeOff, CheckCircle, XCircle, Clock, RefreshCw, ChevronDown } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col items-center gap-1">
      <svg width={72} height={72} viewBox="0 0 72 72">
        <circle cx={36} cy={36} r={r} fill="none" stroke={dark ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.06)'} strokeWidth={6} />
      ...
      <span className="text-xs" style={{ color: dark ? 'rgba(255,255,255,0.5)' : '#6b7280' }}>{label}</span>
    </div>
  );
}
...
export default function DigiLockerPanel({ orgId = 1 }) {
  ...
  useEffect(() => {
    api.getIntegration(orgId).then(data => {
      if (data.integration) {
        ...
        setLiveConfigured(!!data.integration.clientId && data.integration.hasSecret);
      }
    }).catch(() => {});
  }, [orgId]);
  ...
  const validate = () => {
    ...
    else if (!isValidUrl(form.apiUrl.trim())) errs.apiUrl = 'Must be a valid URL';
  ...
  const handleSave = async () => {
    ...
    try {
      await api.saveIntegration({ orgId, clientId: form.clientId.trim(), clientSecret: form.clientSecret || undefined, apiUrl: form.apiUrl.trim(), redirectUri: form.redirectUri.trim(), mode: 'live' });
      setSaveStatus('saved');
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      if (!isAadhaar && !isPan) {
        setVerifyResult({ status: 'Error', errorMsg: 'Enter a 12-digit Aadhaar or 10-character PAN (e.g. ABCDE1234F)' });
        setVerifying(false);
        return;
      }
      const payload = { orgId, ...(isAadhaar ? { aadhaar: v } : { pan: v.toUpperCase() }) };
      const result = await api.verifyLive(payload);
      ...
      setVerifyResult({ status: 'Error', errorMsg: e.message });
    }
    setVerifying(false);
  };
    ...
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs" style={{ color: labelColor }}>Verify Aadhaar & PAN via Live DigiLocker</p>
        </div>
        {liveConfigured && (
            ...
            <AnimatePresence>
              {!liveConfigured && (
                <motion.div initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                  ...
                  <ShieldCheck size={15} style={{ color: '#f59e0b' }} className="flex-shrink-0" />
                  <p className="text-xs flex-1" style={{ color: '#f59e0b' }}>
                    <span className="font-semibold">DigiLocker not configured.</span> Add your Client ID, API URL and Secret in Integration Settings.
                  </p>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <motion.button whileTap={{ scale: 0.96 }} onClick={handleVerify}
                    disabled={verifying || !verifyValue.trim() || !liveConfigured}
                ...
                {verifyResult && (
                  <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                    ...
                    <div className="flex items-center justify-between flex-wrap gap-2">
                      ...
                      {verifyResult.source && <span className="text-xs" style={{ color: labelColor }}>{verifyResult.source}</span>}
                    </div>

                    {verifyResult.status === 'Verified' && (
                      <>
                        <div className="flex gap-4 justify-center py-2">
                          <ScoreRing score={verifyResult.confidenceScore || 98} label="Confidence" color="#10b981" dark={dark} />
                    ...
                    {verifyResult.status === 'Not Verified' && (
                      <p className="text-xs" style={{ color: labelColor }}>No matching record found in DigiLocker.</p>
                    )}
                    {verifyResult.status === 'Error' && (
                      <p className="text-xs" style={{ color: '#ef4444' }}>{verifyResult.errorMsg || 'Verification failed.'}</p>
                    )}
                  </motion.div>
                )}
              </AnimatePresence>
          ...
          <motion.div key="settings" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            <div className="p-5 rounded-2xl space-y-4" style={card}>
              <div className="flex items-center gap-2">
                ...
                <h4 className="text-sm font-bold" style={{ color: headingColor }}>DigiLocker API Configuration</h4>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  <input type={showSecret ? 'text' : 'password'}
                    ...
                    onChange={e => { setForm(f => ({ ...f, clientSecret: e.target.value })); setFieldErrors(fe => ({ ...fe, clientSecret: '' })); }}
