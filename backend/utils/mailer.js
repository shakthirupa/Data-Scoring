...
async function sendEmail(to, subject, message, formLink) {
  const link = formLink || process.env.GOOGLE_FORM_URL || '';
  await transporter.sendMail({
    from: `"DataQuality AI" <${process.env.EMAIL_USER}>`,
    to,
    subject,
        ...
        ${
          link
            ? `<div style="text-align:center;margin:28px 0;">
                <a href="${link}" style="background:#4f46e5;color:#fff;padding:12px 28px;border-radius:8px;text-decoration:none;font-weight:600;font-size:14px;">
                  ✏️ Update Your Details
                </a>
               </div>
               <p style="color:#6b7280;font-size:12px;text-align:center;">Or copy this link: <a href="${link}">${link}</a></p>`
            : ''
        }
